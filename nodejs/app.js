...
var itemCostList = {};
var FREE_TRANS_AMOUNT = 499;    // 設定免運金額
fs.createReadStream('成本清單.csv').pipe(csv()).on('data', function (oneItem) {
    ...
    fs.createReadStream('fafafa1019.shopee-order.20181001-20181031.csv').pipe(iconv.decodeStream('big5')).pipe(csv()).on('data', function (oneOrder) {
        if ( oneOrder['訂單編號'] != '' ){
